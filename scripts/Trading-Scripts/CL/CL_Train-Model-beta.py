...
joined[cont_vars] = joined[cont_vars].bfill().astype('float32')
joined_test[cont_vars] = joined_test[cont_vars].bfill().astype('float32')
...
if config['PARAMS']['need_cont_vars'] == 'TRUE':
    print('Converting dtypes on original df...')
    for col in cat_vars:
...
# for col in [[i for i in df.columns if i.startswith('PrevTarget')]]:
#     df[col] = df[col].ffill()

df[[i for i in df.columns if i.startswith('PrevTarget')]].ffill(inplace=True)
...
if config['PARAMS']['scale_ON'] == 'TRUE':
    ...
    print('Scaling Data...')
    if config['PARAMS']['scaler'] == 'MinMaxScaler':
        scaler = MinMaxScaler().fit(np.array(X_train))
    elif config['PARAMS']['scaler'] == 'StandardScaler':
...
class CalcResults():
    ...
    def initialize_df(self):
        ...
        try:
            ...
            print('Cannot print dtypes of numpy array. PCA is probably running')
            pass
...
def create_oversampled_df(X_train_fit, X_val_fit, HL, need_cont_vars=False, ratio=None):
    ...
    if config['OVERSAMPLING_PARAMS']['OverSampling_algorithm'] == 'SMOTE':
        ...
        raise('OVERSAMPLED_ON and CREATE_OVERSAMPLED_DF_ON must be set to either TRUE or FALSE')
    ...
    if config['PARAMS']['HL_ON'] == 'FALSE':
        ...
        X_resampled, y_resampled = sm.fit_resample(X_temp.reset_index().drop('Datetime', axis=1), y_temp.ravel())
        gc.collect()
...
def main():
    ...
    if config['PARAMS']['HL_ON'] == 'TRUE':
        ...
        if config['PARAMS']['RAW_ON'] == 'TRUE':
            ...
            if config['PARAMS']['scale_ON'] == 'FALSE':
                if config['DIM_REDUCTION_PARAMS']['calc_pca_ON'] == 'FALSE':
                    print('Training RAW MODEL HL...')
                ...
                elif config['DIM_REDUCTION_PARAMS']['calc_pca_ON'] == 'TRUE':
                    print('Training PCA (not scaled)...')
                    pca = PCA(n_components=150).fit(X_train.values)
                    X_train_pca = pca.transform(X_train.values)
                    X_val_pca = pca.transform(X_val.values)
                    X_test_pca = pca.transform(X_test.values)
                    raw_model_HL = TrainModel(model=raw_model,
                                              X_train_fit=X_train_pca, y_train_fit=y_train_HL, X_val_fit=X_val_pca, y_val_fit=y_val_HL,
                                              X_test_unseen=X_test_pca, y_test_unseen=y_test_HL,
                                              need_cont_vars=need_cont_vars_param,
                                              plot_importances=plot_importances_param).train_traditional_ML_model()
                ...
                if config['DIM_REDUCTION_PARAMS']['calc_pca_ON'] == 'FALSE':
                    ...
                    print('Training PCA (scaled)...')
                    pca = PCA(n_components=150).fit(X_train_scaled.values)
                    X_train_pca = pca.transform(X_train_scaled.values)
                    ...
                    CalcResults(model=raw_model_HL, X=X_train_pca, y=y_train_HL, predictions='Train Predictions',
                                stop=stop_HL, strong_cap=strong_cap_HL, med_cap=med_cap_HL,
                                multiplier=multiplier, need_cont_vars=need_cont_vars_param,
                                HL=True, NN=False).calc_results()

                    CalcResults(model=raw_model_HL, X=X_val_pca, y=y_val_HL, predictions='Val Predictions',
                                stop=stop_HL, strong_cap=strong_cap_HL, med_cap=med_cap_HL,
                                multiplier=multiplier, need_cont_vars=need_cont_vars_param,
                                HL=True, NN=False).calc_results()

                    CalcResults(model=raw_model_HL, X=X_test_pca, y=y_test_HL, predictions='Test Predictions',
                                stop=stop_HL, strong_cap=strong_cap_HL, med_cap=med_cap_HL,
                                multiplier=multiplier, need_cont_vars=need_cont_vars_param,
                                HL=True, NN=False).calc_results()
        ...
        if config['OVERSAMPLING_PARAMS']['OVERSAMPLED_ON'] == 'TRUE':
            ...
            if config['OVERSAMPLING_PARAMS']['CREATE_OVERSAMPLED_DF_ON'] == 'TRUE':
                ...
                if config['PARAMS']['scale_ON'] == 'FALSE':
                    ...
                    X_train_oversampled, X_val_oversampled, y_train_oversampled_HL, y_val_oversampled_HL = create_oversampled_df(X_train, X_val, HL=True, need_cont_vars=need_cont_vars_param, ratio=smote_ratio_param, X_train_fit=X_train, X_val_fit=X_val)
                    ...
                    X_train_oversampled, X_val_oversampled, y_train_oversampled_HL, y_val_oversampled_HL = create_oversampled_df(X_train_scaled, X_val_scaled, HL=True, need_cont_vars=need_cont_vars_param, ratio=smote_ratio_param, X_train_fit=X_train_scaled, X_val_fit=X_val_scaled)
                    print(X_train_oversampled.shape, X_val_oversampled.shape, y_train_oversampled_HL.shape, y_val_oversampled_HL.shape)
                ...
                if config['OVERSAMPLING_PARAMS']['read_csv'] == 'TRUE':
                    ...
                    try:
                        ...
                        pass
                elif config['OVERSAMPLING_PARAMS']['read_parquet'] == 'TRUE':
                    oversampled_df = pd.read_parquet(config['PATH']['oversampled_read_df'], use_threads=32)
                    try:
                        oversampled_df = oversampled_df.drop(['Unnamed: 0'], axis=1)
                    except KeyError:
                        pass
        ...
        ########################################################
            ...
            if config['MODEL']['xgbc'] == 'TRUE':
                print('Running XGBClassifier OverSampled-HL')
                model_name = 'XGBC'
                oversampled_model = XGBClassifier(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_HL']))

            if config['MODEL']['lgbmc'] == 'TRUE':
                print('Running LGBMClassifier OverSampled-HL')
                model_name = 'LGBMC'
                oversampled_model = LGBMClassifier(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_HL']))

            if config['MODEL']['gbmc'] == 'TRUE':
                print('Running GradientBoostingClassifier OverSampled-HL')
                model_name = 'GBMC'
                oversampled_model = GradientBoostingClassifier(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_HL']))

            if config['MODEL']['lrc'] == 'TRUE':
                print('Running LogisticRegressionCV OverSampled-HL')
                model_name = 'LRC'
                oversampled_model = LogisticRegressionCV(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_HL']))

            if config['MODEL']['svc'] == 'TRUE':
                print('Running SVC OverSampled-HL')
                model_name = 'SVC'
                oversampled_model = SVC(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_HL']))

            if config['MODEL']['rfc'] == 'TRUE':
                print('Running RandomForestClassifier OverSampled-HL')
                model_name = 'RFC'
                oversampled_model = RandomForestClassifier(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_HL']))




            if config['PARAMS']['scale_ON'] == 'FALSE' and config['DIM_REDUCTION_PARAMS']['calc_pca_ON'] == 'FALSE':
                print('Training RAW MODEL HL...')
                ...
                print('Training PCA OverSampled (not scaled)...')
                pca = PCA(n_components=150).fit(X_train_oversampled.values)
                X_train_pca = pca.transform(X_train_oversampled.values)
                X_val_pca = pca.transform(X_val_oversampled.values)
                ...
                oversampled_model_HL = TrainModel(oversampled_model,
                                            X_train_oversampled, y_train_oversampled_HL, X_val_oversampled, y_val_oversampled_HL,
                                            X_test_pca, y_test_HL,
                                            need_cont_vars=need_cont_vars_param,
                                            plot_importances=plot_importances_param).train_traditional_ML_model()


                CalcResults(oversampled_model_HL, X_train_pca, y_train_HL, predictions='Train Predictions',
                            stop=stop_HL, strong_cap=strong_cap_HL, med_cap=med_cap_HL,
                            multiplier=multiplier, need_cont_vars=need_cont_vars_param,
                            HL=True, NN=False).calc_results()
                CalcResults(oversampled_model_HL, X_val_pca, y_val_HL, predictions='Val Predictions',
                            stop=stop_HL, strong_cap=strong_cap_HL, med_cap=med_cap_HL,
                            multiplier=multiplier, need_cont_vars=need_cont_vars_param,
                            HL=True, NN=False).calc_results()
                CalcResults(oversampled_model_HL, X_test_pca, y_test_HL, predictions='Test Predictions',
                            stop=stop_HL, strong_cap=strong_cap_HL, med_cap=med_cap_HL,
                            multiplier=multiplier, need_cont_vars=need_cont_vars_param,
                            HL=True, NN=False).calc_results()
    ...
    if config['PARAMS']['ACTUAL_ON'] == 'TRUE':
        ...
        if config['OVERSAMPLING_PARAMS']['OVERSAMPLED_ON'] == 'TRUE':
            ...
            if config['OVERSAMPLING_PARAMS']['CREATE_OVERSAMPLED_DF_ON'] == 'TRUE':
                if config['PARAMS']['scale_ON'] == 'FALSE':
                    X_train_oversampled, X_val_oversampled, y_train_oversampled_actual, y_val_oversampled_actual = create_oversampled_df(X_train_fit=X_train, X_val_fit=X_val, HL=False, need_cont_vars=need_cont_vars_param, ratio=smote_ratio_param)
                    print(X_train_oversampled.shape, X_val_oversampled.shape, y_train_oversampled_actual.shape, y_val_oversampled_actual.shape)
                elif config['PARAMS']['scale_ON'] == 'TRUE':
                    X_train_oversampled, X_val_oversampled, y_train_oversampled_actual, y_val_oversampled_actual = create_oversampled_df(X_train_fit=X_train_scaled, X_val_fit=X_val_scaled, HL=False, need_cont_vars=need_cont_vars_param, ratio=smote_ratio_param)
                    print(X_train_oversampled.shape, X_val_oversampled.shape, y_train_oversampled_actual.shape, y_val_oversampled_actual.shape)
            ...
            if config['MODEL']['xgbc'] == 'TRUE':
                ...
                model_name = 'XGBC'
                oversampled_model = XGBClassifier(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_ACTUAL']))

            if config['MODEL']['lgbmc'] == 'TRUE':
                print('Running LGBMClassifier SMOTE-ACTUAL')
                model_name = 'LGBMC'
                oversampled_model = LGBMClassifier(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_ACTUAL']))

            if config['MODEL']['gbmc'] == 'TRUE':
                print('Running GradientBoostingClassifier SMOTE-ACTUAL')
                model_name = 'GBMC'
                oversampled_model = GradientBoostingClassifier(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_ACTUAL']))

            if config['MODEL']['lrc'] == 'TRUE':
                print('Running LogisticRegressionCV SMOTE-ACTUAL')
                model_name = 'LRC'
                oversampled_model = LogisticRegressionCV(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_ACTUAL']))

            if config['MODEL']['svc'] == 'TRUE':
                print('Running SVC SMOTE-ACTUAL')
                model_name = 'SVC'
                oversampled_model = SVC(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_ACTUAL']))

            if config['MODEL']['rfc'] == 'TRUE':
                print('Running RandomForestClassifier SMOTE-ACTUAL')
                model_name = 'RFC'
                oversampled_model = RandomForestClassifier(**ast.literal_eval(config['PARAMS']['oversampled_model_hyperparams_ACTUAL']))
    ...
    try:
        # SAVE RAW
        if config['PARAMS']['save_raw_model'] == 'TRUE':
            if config['PARAMS']['HL_ON'] == 'TRUE':
        ...
        if config['OVERSAMPLING_PARAMS']['save_oversampled_model'] == 'TRUE' and config['OVERSAMPLING_PARAMS']['OVERSAMPLED_ON'] == 'TRUE':
            if config['PARAMS']['HL_ON'] == 'TRUE':
                pickle.dump(oversampled_model_HL, open(config['PATH']['model_outpath'] + config['NAME']['product'] + model_name + '_oversampled-HL_' + str(datetime.datetime.today().date()) + '.pickle.dat', 'wb'))
                ...
                X_train.to_csv(config['PATH']['model_outpath'] + config['NAME']['product'] + model_name + '_oversampled-HL_X_train_' + str(datetime.datetime.today().date()) + '.csv')

            if config['PARAMS']['ACTUAL_ON'] == 'TRUE':
                pickle.dump(oversampled_model_actual, open(config['PATH']['model_outpath'] + config['NAME']['product'] + model_name + '_oversampled-ACTUAL_' + str(datetime.datetime.today().date()) + '.pickle.dat', 'wb'))
                ...
                X_train.to_csv(config['PATH']['model_outpath'] + config['NAME']['product'] + model_name + '_oversampled-ACTUAL_X_train_' + str(datetime.datetime.today().date()) + '.csv')

    except UnboundLocalError:
        raise("You probably didn't specify RAW_ON or SMOTE_ON in the config file.")

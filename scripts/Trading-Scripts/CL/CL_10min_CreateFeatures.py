...
config.read('/home/melgazar9/Trading/TD/Scripts/Trading-Scripts/CL/scripts/CL_10min_CreateFeatures.ini')
...
#
#     s = df['1minClose'] * df['1minVolume']
#     s = s.groupby(pd.Grouper(freq=price_groupby_freq)).cumsum()
#     volume_groupby_freq = df.groupby(pd.Grouper(freq=volume_groupby_freq))['1minVolume'].cumsum()
#
...
def main():
    ...
    if config['PARAMS']['read_csv'] == 'TRUE':
        ...
        df_5min = pd.read_feather(config['PATH']['read_file'])
    elif config['PARAMS']['read_parquet'] == 'TRUE':
        print('Reading Parquet File...')
        df_5min = pd.read_parquet(config['PATH']['read_file'])
    ...
    if config['PARAMS']['lookback_increment_timesteps_ON'] == 'TRUE':
        ...
        dfs = {f'{i}min' : get_df_init(df_5min, f'{i}min') for i in ast.literal_eval(config['PARAMS']['lookback_timesteps_list'])}
    ...
    if config['PARAMS']['save_df'] == 'TRUE':
        ...
        if config['PARAMS']['write_feather'] == 'TRUE':
            ...
            df.reset_index().to_feather(config['PATH']['save_df_path'] + config['PARAMS']['product'] + '_10min_FULL_' + str(datetime.datetime.today().date()) + '.feather')
        if config['PARAMS']['write_csv'] == 'TRUE':
            df.to_parquet(config['PATH']['save_df_path'] + config['PARAMS']['product'] + '_10min_FULL_' + str(datetime.datetime.today().date()) + '.parquet')

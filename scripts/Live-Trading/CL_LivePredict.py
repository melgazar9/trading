...
config_path = '/home/melgazar9/Trading/TD/Live-Trading/CL/scripts/CL_{}_TD_TRADITIONAL.ini'.format(sys.argv[1])
config.read(config_path)

print('******USING MODEL', config['MODEL']['model_name'] + '******')

pnl_multiplier = float(config['PARAMS']['pnl_multiplier'])
tick_multiplier = float(config['PARAMS']['tick_multiplier'])
...
def get_historical_df(raw_data):
    ...
    cols = {'Key':'/' + config['NAME']['product'], '0':'Datetime','1':'1minOpen','2':'1minHigh','3':'1minLow','4':'1minClose','5':'1minVolume'}
...
def calc_pnl_with_stop_with_cap(model, df, X, y, stop, strong_cap, med_cap, predictions, model_name):
    ...
    print(predictions + ' P/L BEFORE FEES:', int(float(np.sum(results[model_name + ' P/L'])*1000)))
    print(predictions + ' P/L AFTER FEES:', int(float(np.sum(results[model_name + ' P/L'])*1000) - float((round_trip_fee)*len(results.loc[results[predictions]!=2])))) # subtract fees
...
#     df_last_row = pd.DataFrame(index=[pd.to_datetime(df_5min.index[-1] + pd.Timedelta(minutes=10))])
    ...
    df_last_row = pd.DataFrame(index=[pd.to_datetime(df_1min.index[-1] + pd.Timedelta(minutes=int(config['MODEL']['model_name'][0:2])))])
...
class CalcResults():
    ...
    def __init__(self, model, df, X, y, predictions, stop, strong_cap, med_cap, pnl_multiplier, need_cont_vars, HL, NN=False):
    ...
    def calc_predictions(self):
        ...
        # This parameter means (for example) i can only predict sell if highest probability is sell and second highest probability is sell
        # for p in probs:
        #
        ...
        #
        #
        # if config['PARAMS']['PredProb_OrderedPred_ON'] == 'FALSE':
    ...
    def calc_pnl_traditional_onelot_actual(self):
        ...
        print(self.predictions + ' P/L BEFORE FEES Actual:', int(float(np.sum(results[self.predictions.split(' ')[0] + ' P/L Actual'])*self.pnl_multiplier)))
        print(self.predictions + ' P/L AFTER FEES Actual:', int(float(np.sum(results[self.predictions.split(' ')[0] + ' P/L Actual'])*self.pnl_multiplier) - float((round_trip_fee)*len(results.loc[results[self.predictions]!=2])))) # subtract fees
        ...
        print('Class 4:', round(pnl_4['Actual P/L'].sum()*self.pnl_multiplier, -1))
        print('Class 3:', round(pnl_3['Actual P/L'].sum()*self.pnl_multiplier, -1))
        print('Class 1:', round(pnl_1['Actual P/L'].sum()*self.pnl_multiplier, -1))
        print('Class 0:', round(pnl_0['Actual P/L'].sum()*self.pnl_multiplier, -1))
    ...
    def calc_pnl_traditional_2to1_actual(self):
        ...
        print(self.predictions + ' P/L HL BEFORE FEES:', int(float(np.sum(results[self.predictions.split(' ')[0] + ' P/L HL'])*self.pnl_multiplier)))
        print(self.predictions + ' P/L HL AFTER FEES:', int(float(np.sum(results[self.predictions.split(' ')[0] + ' P/L HL'])*self.pnl_multiplier - float((round_trip_fee)*len(results.loc[results[self.predictions]!=2]))))) # subtract fees
...
#         results = pd.concat([results, pnl, pd.DataFrame(lst2, index=results.index)], axis=1)
        ...
        temp = int(float(np.sum(results['HL P/L'])*self.pnl_multiplier))
        print(self.predictions + ' P/L HL BEFORE FEES:', temp)
        print(self.predictions + ' P/L HL AFTER FEES:', temp - int((round_trip_fee)*len(results.loc[results[self.predictions]!=2]))) # subtract fees
        ...
        print('Class 4:', round(pnl_4['HL P/L'].sum()*self.pnl_multiplier, -1))
        print('Class 3:', round(pnl_3['HL P/L'].sum()*self.pnl_multiplier, -1))
        print('Class 1:', round(pnl_1['HL P/L'].sum()*self.pnl_multiplier, -1))
        print('Class 0:', round(pnl_0['HL P/L'].sum()*self.pnl_multiplier, -1))

        print('P/L Assuming All Bad Fills (Before Fees):', int(float(temp - float(len(results[results[self.predictions]!=2]))*tick_multiplier))) # PnL before fees - NumTrades*pnl_multiplier -> This assumes losing one extra tick per trade
        print('P/L Assuming All Bad Fills (After Fees):', int(float(temp - float(len(results[results[self.predictions]!=2]))*tick_multiplier - (round_trip_fee)*len(results.loc[results[self.predictions]!=2]))))
...
def main():
    ...
    if config['PARAMS']['dropped_connection'] == 'TRUE':
        ...
        df_1min['Symbol'] = '/' + config['NAME']['product']
        # print(df_1min.get_dtype_counts())

    last_row_1min = pd.DataFrame(get_last_row(df_1min, '1min').iloc[-1]).T
    last_row_1min.index = pd.DatetimeIndex([df_1min.index[-1] + pd.Timedelta(minutes=int(config['MODEL']['model_name'][0:2]))])
    last_row_1min['Symbol'] = symbol.iloc[-1]
    ...
    if config['MODEL']['model_name'] != '10min':
        df[Actual_Move] = df[['Prev' + Actual_Move.strip('Actual')]].resample(config['MODEL']['model_name']).first().rename(columns={'Prev' + Actual_Move.strip('Actual') : Actual_Move}).shift(-1)#.ffill()
        df[Actual_HighMove] = df[['Prev' + Actual_HighMove.strip('Actual')]].resample(config['MODEL']['model_name']).first().rename(columns={'Prev' + Actual_HighMove.strip('Actual') : Actual_HighMove}).shift(-1)#.ffill()
        df[Actual_LowMove] = df[['Prev' + Actual_LowMove.strip('Actual')]].resample(config['MODEL']['model_name']).first().rename(columns={'Prev' + Actual_LowMove.strip('Actual') : Actual_LowMove}).shift(-1)#.ffill()
    ...
    if config['MODEL']['model_name'] != '10min':
        df = pd.concat([df.iloc[0:-2].dropna(), df.iloc[-2:]], axis=0).ffill()
    else:
    ...
    try:
        ...
        df.drop([Actual_Move + 'RollingSum_Window4',
                 ...
                 Actual_Move + 'RollingMin_Window4',
                 Actual_HighMove + 'RollingSum_Window4',
                 Actual_HighMove + 'RollingMean_Window4',
                 Actual_HighMove + 'RollingStd_Window4',
                 Actual_HighMove + 'RollingMax_Window4',
                 Actual_HighMove + 'RollingMin_Window4',
                 Actual_LowMove + 'RollingSum_Window4',
                 Actual_LowMove + 'RollingMean_Window4',
                 Actual_LowMove + 'RollingStd_Window4',
                 Actual_LowMove + 'RollingMax_Window4',
                 Actual_LowMove + 'RollingMin_Window4'], axis=1, inplace=True)
    except:
    ...
    if config['MODEL']['model_name'] != '10min':
        df = df.resample(config['MODEL']['model_name']).first()
    print(df)

    # Add Previous Targets HL
    ...
    # Add Previous Targets Actual
    for i in range(int(config['PARAMS']['min_target_lookback']), int(config['PARAMS']['max_target_lookback']), int(config['PARAMS']['target_lookback_increment'])):
        target_actual['PrevTarget_Actual' + str(i)] = target_actual['Target_Actual'].shift(i)

    target_HL = target_HL.fillna(2).astype('int')
    target_actual = target_actual.fillna(2).astype('int')


    df = pd.concat([pd.DataFrame(target_HL[[i for i in target_HL.columns if i.startswith('Prev')]], index=df.index), df], axis=1)
    df = pd.concat([pd.DataFrame(target_actual[[i for i in target_actual.columns if i.startswith('Prev')]], index=df.index), df], axis=1)
    ...
    df.loc[df.index.is_year_start, 'IsYearStart_True'] = 1
    df['IsYearStart_True'].fillna(0, inplace=True)
    df['IsYearStart_True'] = df['IsYearStart_True'].astype('category').cat.as_ordered()

    df.loc[df.index.is_year_end, 'IsYearEnd_True'] = 1
    df['IsYearEnd_True'].fillna(0, inplace=True)
    df['IsYearEnd_True'] = df['IsYearEnd_True'].astype('category').cat.as_ordered()



    for y in range(2015, 2025):
        df.loc[df.index.year == y, 'Year_'+str(y)] = 1
        df.loc[df.index.year != y, 'Year_'+str(y)] = 0
    ...
    if config['PARAMS']['scale'] == 'TRUE':
        ...
        # load scaler
        scaler = pickle.load(open(config['MODEL']['scaler']), 'rb')
    ...
    if config['PARAMS']['HL_ON'] == 'TRUE':
        if xgbc_on == True:
            print('SMOTE HL XGBC')
            hl_results_xgbc = CalcResults(smote_xgbc_HL, df, X.astype('float32'), y_HL, 'HL Predictions', stop_HL, strong_cap_HL, med_cap_HL, pnl_multiplier, need_cont_vars=False, HL=True).calc_results()
        ...
        if gbmc_on == True:
            print('SMOTE HL GBMC')
            hl_results_gbmc = CalcResults(smote_gbmc_HL, df, X, y_HL, 'HL Predictions', stop_HL, strong_cap_HL, med_cap_HL, pnl_multiplier, need_cont_vars=False, HL=True).calc_results()
        ...
        if lgbmc_on == True:
            ...
            hl_results_lgbmc = CalcResults(smote_lgbmc_HL, df, X, y_HL, 'HL Predictions', stop_HL, strong_cap_HL, med_cap_HL, pnl_multiplier, need_cont_vars=False, HL=True).calc_results()
    ...
    if config['PARAMS']['ACTUAL_ON'] == 'TRUE':
        if xgbc_on == True:
            ...
            actual_results_xgbc = CalcResults(smote_xgbc_actual, df, X, y_Actual, 'Actual Predictions', stop_actual, strong_cap_actual, med_cap_actual, pnl_multiplier, need_cont_vars=False, HL=False).calc_results()

        if gbmc_on == True:
            print('SMOTE ACTUAL GBMC')
            actual_results_gbmc = CalcResults(smote_gbmc_actual, df, X, y_Actual, 'Actual Predictions', stop_actual, strong_cap_actual, med_cap_actual, pnl_multiplier, need_cont_vars=False, HL=False).calc_results()

        if lgbmc_on == True:
            print('SMOTE ACTUAL LGBMC')
            actual_results_lgbmc = CalcResults(smote_lgbmc_actual, df, X, y_Actual, 'Actual Predictions', stop_actual, strong_cap_actual, med_cap_actual, pnl_multiplier, need_cont_vars=False, HL=False).calc_results()
    ...
    if config['SAVE']['save_df'] == 'TRUE':
        ...
        df.to_csv(config['SAVE']['save_path'] + config['NAME']['product'][0:2] + '_' + config['MODEL']['model_name'] + '_FULL_results_' + str(datetime.datetime.today().date()) + '.csv')
